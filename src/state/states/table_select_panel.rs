...
impl TableSelectPanel {
    ...
    /// Get the tables for the current database
    fn get_current_tables<'a>(&self, context: &'a StateContext) -> Vec<&'a crate::db::TableInfo> {
        if let Some(current_db) = &context.global_state.selected_database {
            if let Some(db_info) = context.database_manager.get_databases()
                .iter()
    ...
    fn update_selected_table(&self, context: &mut StateContext) {
        ...
        let table_info = {
            ...
            tables.get(self.selected_index).map(|table| (table.name.clone(), table.column_count))
        };
        
        if let Some((table_name, column_count)) = table_info {
            context.set_selected_table(Some(table_name.clone()));
            context.action_logger.log_info(&format!("Selected table: {}", table_name));
            context.set_status_message(format!("Table: {} ({} columns)", table_name, column_count));
        }
    }
}
...
impl UIState for TableSelectPanel {
    ...
    fn handle_event(&mut self, event: KeyEvent, context: &mut StateContext) -> StateTransition {
        match event.code {
            ...
            KeyCode::Char('v') => {
                ...
                if self.get_current_tables(context).get(self.selected_index).is_some() {
            ...
            KeyCode::Char('r') => {
                ...
                if let Some(db_name) = &context.global_state.selected_database {
                    ...
                    context.action_logger.log_info(&format!("Refreshing tables for database: {}", db_name));
                    context.add_notification(Notification::info("Table list refreshed".to_string()));
                }
                StateTransition::Stay
            }
    ...
    fn on_enter(&mut self, context: &mut StateContext) -> Result<()> {
        context.action_logger.log_debug("Entered TableSelectPanel");
        self.update_selected_table(context);
        Ok(())
    }
