...
pub struct TableDataViewer {
    ...
    // Column visibility state - tracks hidden columns
...
impl TableDataViewer {
    ...
    fn update_viewport(&mut self, available_width: usize) {
        if let Some(ref data) = self.table_data {
            ...
            if visible_indices.is_empty() && self.scroll_x < data.columns.len() {
                ...
                used_width = *self.viewport.column_widths.get(self.scroll_x).unwrap_or(&min_col_width);
            }
            ...
            self.viewport.rightmost_column_index = visible_indices.last().copied().unwrap_or(self.scroll_x);
        }
    }
    ...
    pub fn handle_column_reorder_completion(&mut self, context: &mut StateContext) {
        ...
        if let Some(ref current_col) = self.selected_column.clone() {
            if self.is_column_hidden_by_name_with_context(current_col, context) {
                context.action_logger.log_debug(&format!("Column {} is now hidden, adjusting selection", current_col));
                
                // Store current column index for reference
                let current_col_index = self.get_selected_column_index();
                
                // Try to move left first (preferred behavior)
                if let Some(prev_col) = self.get_prev_visible_column_with_context(current_col, context) {
                    self.selected_column = Some(prev_col);
                    context.action_logger.log_debug("Moved selection to previous visible column");
                } else if let Some(next_col) = self.get_next_visible_column_with_context(current_col, context) {
                    // No previous column, try next column
                    self.selected_column = Some(next_col);
                    context.action_logger.log_debug("Moved selection to next visible column");
                } else {
                    // No visible columns at all, select first visible column if any exist
                    self.selected_column = self.get_first_visible_column_with_context(context);
                    context.action_logger.log_debug("No adjacent visible columns, selected first visible column");
                }
                
                // Don't adjust viewport - keep current scroll position
                // Only ensure column is visible if it's way off screen
                if let Some(new_column_index) = self.get_selected_column_index() {
                ...
                context.action_logger.log_debug("Current column is still visible, no adjustment needed");
            }
        } else {
            // No current selection, select first visible column but don't change viewport
            self.selected_column = self.get_first_visible_column_with_context(context);
            context.action_logger.log_debug("No current selection, selected first visible column");
        }
    }
    ...
    fn build_table_query(&self, table_name: &str, context: &StateContext) -> String {
        ...
        let select_clause = if let Some(ordered_columns) = context.global_state.column_order.get(table_name) {
            // Filter out hidden columns
            let visible_columns: Vec<String> = if let Some(hidden_columns) = context.global_state.hidden_columns.get(table_name) {
                ordered_columns.iter()
        ...
        match row.get::<_, f64>(index) {
            Ok(v) => {
                if v.is_nan() {
                    ...
                    Ok(if v.is_sign_positive() { "Infinity" } else { "-Infinity" }.to_string())
                } else {
                    Ok(v.to_string())
                }
            },
...
impl UIState for TableDataViewer {
    fn render(&mut self, frame: &mut Frame, area: Rect, is_active: bool, context: &mut StateContext) -> Result<()> {
        ...
        if self.table_data.is_some() {
            ...
            let header_cells: Vec<Cell> = visible_cols.iter().map(|&col_idx| {
                ...
                if let Some((direction, order)) = context.global_state.is_column_sorted(col_name) {
                ...
                let truncated_header = if header_text.chars().count() > *col_width {
                    format!("{}...", header_text.chars().take(col_width.saturating_sub(3)).collect::<String>())
                } else {
                    header_text
                };
                
                let cell = Cell::from(truncated_header).style(Style::default().add_modifier(Modifier::BOLD));
                if Some(col_name.clone()) == self.selected_column {
            ...
            let rows: Vec<Row> = data.rows
                ...
                .map(|(i, row)| {
                    ...
                    let cells: Vec<Cell> = visible_cols.iter().map(|&col_idx| {
                        let empty_string = String::new();
                        let cell_content = row.get(col_idx).unwrap_or(&empty_string);
                        let column_name = data.columns.get(col_idx).cloned().unwrap_or_default();
                        let is_selected_column = Some(column_name) == self.selected_column;
                        ...
                        let truncated_content = if cell_content.chars().count() > *col_width {
                            format!("{}...", cell_content.chars().take(col_width.saturating_sub(3)).collect::<String>())
                        } else {
                            cell_content.clone()
                        };
                        ...
                        if is_current_cell {
                            ...
                            Cell::from(truncated_content)
                                .style(Style::default().bg(Color::Gray).fg(Color::Black).add_modifier(Modifier::BOLD))
    ...
    fn handle_event(&mut self, event: KeyEvent, context: &mut StateContext) -> StateTransition {
        match event.code {
            ...
            KeyCode::Right | KeyCode::Char('l') => {
                ...
                if let Some(ref current_col) = self.selected_column.clone() {
                    if let Some(next_col) = self.get_next_visible_column_with_context(current_col, context) {
                        self.selected_column = Some(next_col);
                        if let Some(column_index) = self.get_selected_column_index() {
                            self.ensure_column_visible(column_index);
                        }
            ...
            KeyCode::Down | KeyCode::Char('j') => {
                ...
                let max_row = if let Some(total) = self.total_row_count {
                    ...
                    self.table_data.as_ref().map(|d| d.rows.len().saturating_sub(1)).unwrap_or(0)
                };
            ...
            KeyCode::Char('m') => {
                ...
                // Update global state with current selected column
                context.global_state.current_selected_column = self.selected_column.clone();
            ...
            KeyCode::Char('a') => {
                ...
                if let Some(column_name) = &self.selected_column {
                    context.global_state.toggle_column_sort(column_name.clone(), true);
            ...
            KeyCode::Char('A') => {
                ...
                if let Some(column_name) = &self.selected_column {
                    context.global_state.toggle_column_sort(column_name.clone(), false);
                    self.fetch_table_data(context); // Refresh data with new sort
                }
                StateTransition::Stay
            }
            ...
            KeyCode::Char('f') => {
                // Start column filter
                if let Some(column_name) = &self.selected_column {
                    context.global_state.start_column_filter(column_name.clone());
            ...
            KeyCode::Char('L') => {
                ...
                self.selected_column = self.get_last_visible_column_with_context(context);
                if let Some(column_index) = self.get_selected_column_index() {
                    self.ensure_column_visible(column_index);
                }
                StateTransition::Stay
    ...
    fn on_enter(&mut self, context: &mut StateContext) -> Result<()> {
        ...
        if context.global_state.selected_table.is_some() {
            ...
            context.action_logger.log_debug("No table selected, skipping data fetch");
            self.table_data = None;
        }
        Ok(())
    }
    ...
    fn handle_modal_completion(&mut self, modal_name: &str, context: &mut StateContext) -> Result<()> {
        match modal_name {
            "ColumnReorderMode" => {
                // Handle column reorder completion with proper selection adjustment
                self.handle_column_reorder_completion(context);
            }
            _ => {
                // Other modals don't need special handling
            }
        }
        Ok(())
    }
...
impl TableDataViewer {
    ...
    fn get_next_visible_column_with_context(&self, current_column: &str, context: &StateContext) -> Option<String> {
        if let Some(ref data) = self.table_data {
            if let Some(current_idx) = data.columns.iter().position(|name| name == current_column) {
                for idx in (current_idx + 1)..data.columns.len() {
                    if let Some(name) = data.columns.get(idx) {
                        if !self.is_column_hidden_by_name_with_context(name, context) {
    ...
    fn get_prev_visible_column_with_context(&self, current_column: &str, context: &StateContext) -> Option<String> {
        if let Some(ref data) = self.table_data {
            if let Some(current_idx) = data.columns.iter().position(|name| name == current_column) {
    ...
    fn get_first_visible_column_with_context(&self, context: &StateContext) -> Option<String> {
        if let Some(ref data) = self.table_data {
    ...
    fn get_last_visible_column_with_context(&self, context: &StateContext) -> Option<String> {
        if let Some(ref data) = self.table_data {
    ...
    fn get_selected_column_index(&self) -> Option<usize> {
        if let (Some(ref selected_col), Some(ref data)) = (&self.selected_column, &self.table_data) {
    ...
    /// Check if a column is hidden by name (deprecated - use with_context version)
    fn is_column_hidden_by_name(&self, _column_name: &str) -> bool {
        // This method needs access to context, which we don't have here
        // For now, return false - the column hiding is handled in build_table_query
        false
    }
    
    /// Get the next visible column after the current one (backwards compatibility)
    fn get_next_visible_column(&self, current_column: &str) -> Option<String> {
        // This method doesn't have context access, so it can't check hidden columns properly
        // It will return the next column regardless of visibility
        if let Some(ref data) = self.table_data {
            if let Some(current_idx) = data.columns.iter().position(|name| name == current_column) {
                for idx in (current_idx + 1)..data.columns.len() {
                    if let Some(name) = data.columns.get(idx) {
    ...
    /// Get the previous visible column before the current one (backwards compatibility)
    fn get_prev_visible_column(&self, current_column: &str) -> Option<String> {
        // This method doesn't have context access, so it can't check hidden columns properly
        // It will return the previous column regardless of visibility
        if let Some(ref data) = self.table_data {
            if let Some(current_idx) = data.columns.iter().position(|name| name == current_column) {
                for idx in (0..current_idx).rev() {
                    if let Some(name) = data.columns.get(idx) {
    ...
    /// Get the first visible column (backwards compatibility)
    fn get_first_visible_column(&self) -> Option<String> {
        // This method doesn't have context access, so it can't check hidden columns properly
        // It will return the first column regardless of visibility
        if let Some(ref data) = self.table_data {
            data.columns.first().cloned()
        } else {
            None
        }
    }
    
    /// Get the last visible column (backwards compatibility)
    fn get_last_visible_column(&self) -> Option<String> {
        // This method doesn't have context access, so it can't check hidden columns properly
        // It will return the last column regardless of visibility
        if let Some(ref data) = self.table_data {
            data.columns.last().cloned()
        } else {
            None
        }
    }
    ...
    fn is_column_hidden_by_name_with_context(&self, column_name: &str, context: &StateContext) -> bool {
        if let Some(table_name) = &context.global_state.selected_table {
    ...
    pub fn adjust_selection_after_column_hide(&mut self, context: &StateContext) {
