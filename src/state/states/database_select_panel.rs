...
pub struct DatabaseSelectPanel {
    ...
    // hey claude! the database selection panel should not take up that much vertical space
    // it should be essentially a "dropdown" that prints the currently selected database
    // when the user hits j/k when the database selector is active
    // a dropdown should popup that lists databases and lets the user connect to a new database
    // note that we will likely need to create a new UIState for this dropdown that handles
    // database listing and index syncing and the like.
    ...
    fn update_selected_database(&self, context: &mut StateContext) {
        ...
        let db_info = {
            ...
            databases
                ...
                .map(|db| (db.name.clone(), db.is_memory))
        };
        ...
        if let Some((db_name, _is_memory)) = db_info {
            ...
            if let Err(e) = context.database_manager.set_current_database(&db_name) {
                ...
                context.add_notification(Notification::error(format!(
                    "Failed to switch to database: {e}"
                )));
            } else {
                tracing::info!("Switched to database: {}", db_name);
...
impl UIState for DatabaseSelectPanel {
    ...
    fn handle_event(&mut self, event: KeyEvent, context: &mut StateContext) -> StateTransition {
        match event.code {
            ...
            KeyCode::Enter => {
                self.update_selected_database(context);
                // The LeftSidebarPanel will handle the switch to table panel
                StateTransition::Stay
            }
            ...
            KeyCode::Char('o') => {
                ...
                tracing::info!("File browser not yet implemented");
                context
                    .add_notification(Notification::info("File browser coming soon".to_string()));
                StateTransition::Stay
            }
            KeyCode::Char('s') => {
                ...
                if let Some(db) = databases.get(self.selected_index) {
                    if db.is_memory {
                        ...
                        tracing::info!("Save database {} to file", db.name);
                        context.add_notification(Notification::info(
                            "Save functionality coming soon".to_string(),
                        ));
                    } else {
                        context.add_notification(Notification::info(
                            "Database already saved to disk".to_string(),
                        ));
                    }
                }
                StateTransition::Stay
            ...
            KeyCode::Delete | KeyCode::Char('d') => {
                ...
                if let Some(db) = databases.get(self.selected_index) {
                    ...
                    StateTransition::Push(Box::new(super::DeleteConfirmation::new_for_database(
                        db.name.clone(),
                    )))
                } else {
                    StateTransition::Stay
    ...
    fn on_enter(&mut self, context: &mut StateContext) -> Result<()> {
        tracing::debug!("Entered DatabaseSelectPanel");
